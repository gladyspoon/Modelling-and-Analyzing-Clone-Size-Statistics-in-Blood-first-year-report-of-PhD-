...
Population_size=10^6                                 #is the starting population size of clone
hitchhiker_cutoff=0.98;
benefit=0.05;
...
#divide: propagate growth of each clone
          ...
          for (i in 1:length(Bio_system)) {
            ...
            if(Bio_system[[i]][[2]]!=0){                           #only divide non-extinct clones
              ...
              if(Bio_system[[i]][[2]]<population_limit){
...
for (m in 2:last_mutation_ID) {
    ...
    for (k in 1:length(CloneID)) {
      ...
      if(CloneID[[k]][[1]]==m){
        ...
        mutation_frequency_beneficial[[m]]=mutation_frequency_beneficial[[m]]+Size
          ...
          if(k<where_first_beneficial_mutation_comes_in & where_first_beneficial_mutation_comes_in!=0){ Type_of_neutral_mutation="Beneficial later"}
          if(k>where_first_beneficial_mutation_comes_in & where_first_beneficial_mutation_comes_in!=0){ Type_of_neutral_mutation="Beneficial first"}
          if(where_first_beneficial_mutation_comes_in==0){ Type_of_neutral_mutation="Not Hitchhiker"}
                       #print(paste("The neutral mutation ", m," belongs to the type", Type_of_neutral_mutation))
  ...
  if(mutation_frequency_neutral[[m]]!=0){
  ...
  if(ratio1 < hitchhiker_cutoff){mutation_frequency_neutral_beneficial_later[[m]]=mutation_frequency_neutral[[m]]}
  else{mutation_frequency_neutral_not_hitchhiker[[m]]=mutation_frequency_neutral[[m]]}
  
  #mutation_frequency_neutral_beneficial_first[[m]]=mutation_frequency_neutral[[m]]
  }
...
plot7<- qplot(logtotal, geom="histogram",
      #log = "x",
...
plot8<- qplot(
              ...
              main = paste(
                           ...
                           "\n U =", u/(1+ratio_neutral_beneficial),  "           s =", benefit,"          lifespan =",lifespan,"          population size =",
                           Population_size,"\n Number of Participants =",
                           Number_of_Participants,"          code ran for ",code_ran_for, "hours"))

grid.arrange(plot1,plot2,plot3,plot4,plot6,plot7,plot8,nrow = 4)
